...
def mcode(ite):
    R = 0.5
    testfile = open("SVpr%f_%f.txt" % (R,ite), 'w')
    ...
    Xnew4 = X[:, 5040:6720]
    Xnew5 = X[:, 6720:6745]
    X = np.concatenate((Xnew2,Xnew3, Xnew4), axis=1)
    #Xnew6 = X[:, 6745:]
    ...
    """
    # Late RF
    resall1 = np.column_stack((pred2, pred3,pred4))
    ...
    elaterf.append(LRF)
    # Late RF dis
    resall = np.column_stack((pre2, pre3, pre4))
    ...
    #multi view
    X_features_trainm = (
                            X_features_train2+  X_features_train4 + X_features_train3  ) / 3
    X_features_testm = (
                           X_features_test2+ X_features_test4 + X_features_test3  ) /3
