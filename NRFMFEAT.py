...
def mcode(ite):
    R = 0.5
    testfile = open("FinalALLNRFIDH1%f_%f.txt" % (R,ite), 'w')
    ...
    array = dataframe.values
    Xnew6 = array
    X = np.concatenate((Xnew1, Xnew2, Xnew3, Xnew4, Xnew5, Xnew6), axis=1)
    ll = list(range(10))
    Y = []
    for i in ll:
        Y = Y + [i] * 200
    ...
    train_indices, test_indices = splitdata(X=X, Y=Y, ratio=0.3, seed=100)
    X = X[train_indices]
    Y = Y[train_indices]
    print(X.shape)
    Xnew1 = Xnew1[train_indices]
    Xnew2 = Xnew2[train_indices]
    Xnew3 = Xnew3[train_indices]
    Xnew4 = Xnew4[train_indices]
    Xnew5 = Xnew5[train_indices]
    Xnew6 = Xnew6[train_indices]
