...
def mcode(ite):
    R = 0.5
    testfile = open("SYS800IDH1%f_%f.txt" % (R,ite), 'w')
    ...
    for ii in range(31):
        ...
        X_features_train1, X_features_test1, w1, pred1 = rf_dis(n_trees=500, X=Xdata, Y=Y, train_indices=train_indices,
                                                                test_indices=test_indices, seed=seed)
        str0 = X_features_train1.shape[0]
        str1 = X_features_train1.shape[1]
        ste0 = X_features_test1.shape[0]
        ste1 = X_features_test1.shape[1]
    ...
    """
    X_features_trainm = np.zeros(str0,str1)
    X_features_testm = np.zeros(ste0,ste1)
    for ss in range(31):
        X_features_trainm = X_features_trainm+X_features_train[ss]
        X_features_testm = X_features_testm + X_features_test[ss]
    X_features_trainm = X_features_trainm/31
    X_features_testm = X_features_testm/31
