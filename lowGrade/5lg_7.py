...
def mcode(ite):
    R = 0.7
    testfile = open("3V5nonIDH1%f_%f.txt" % (R,ite), 'w')
    ...
    Xnew4 = X[:, 5040:6720]
    Xnew5 = X[:, 6720:6745]
    X = np.concatenate((X[:,0:5040], X[:, 6720:6745]), axis=1)
    #Xnew6 = X[:, 6745:]
    ...
    """
    # Late RF
    resall1 = np.column_stack((pred1, pred2, pred3,pred5))
    ...
    elaterf.append(LRF)
    # Late RF dis
    resall = np.column_stack((pre1, pre2, pre3, pre5))
    ...
    #multi view
    X_features_trainm = (
                            X_features_train1 + X_features_train2 + X_features_train3 +X_features_train5 ) / 4
    X_features_testm = (
                           X_features_test1 + X_features_test2 + X_features_test3  +X_features_test5) /4
